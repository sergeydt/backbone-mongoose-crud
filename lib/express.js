...
var hasLogFile = fs.existsSync('./logs/crud.log')

var logFile = hasLogFile ? fs.createWriteStream('./logs/crud.log', {
    flags: 'a+'
}) : null; //use {flags: 'w'} to open in write mode

crudApp.configure('development', function(){
    var loggerOptions = logFile ? {stream: logFile} : {};
    crudApp.use(express.logger(loggerOptions));
	...
	if (mongoose.models.hasOwnProperty(modelName)) {
		...
		require('./model')(modelName, req.method, model, function(ret){
			if (ret.length){
				...
				if (ret.toObject)  ret = ret.toObject()
				res.json(ret);
			}
		});
	}
